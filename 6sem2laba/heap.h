...
private:
	...
	{
		if (i && binaryHeap[Parent(i)].first > binaryHeap[i].first)
		{
			swap(binaryHeap[i], binaryHeap[Parent(i)]);
	...
	void heapifyDown(int i)
	{
		...
		if (right < binaryHeap.size() && binaryHeap[right].first < binaryHeap[min].first)
		{
			min = right;
		}

		if (left < binaryHeap.size() && binaryHeap[left].first < binaryHeap[i].first)
		{
			min = left;
		}
		...
		if (min != i)
		{
			swap(binaryHeap[i], binaryHeap[min]);
			heapifyDown(min);
		}
	}
...
public:
	...
	void show()
	{
		int i = 0;
		int k = 1;
		while (i < binaryHeap.size()) {
			while ((i < k) && (i < binaryHeap.size())) {
				cout << binaryHeap[i].first << " ";
				i++;
			}
			cout << endl;
			k = k * 2 + 1;
